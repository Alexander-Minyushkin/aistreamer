...
Original code taken here
https://github.com/spiglerg/RNN_Text_Generation_Tensorflow and modified.
...
class ModelNetwork:
    def __init__(self, in_size, lstm_size, num_layers, out_size, session, learning_rate=0.003, name="rnn"):
        ...
        with tf.variable_scope(self.scope):
            ...
            self.xinput = tf.placeholder(tf.float32, shape=(
                None, None, self.in_size), name="xinput")
            self.lstm_init_value = tf.placeholder(tf.float32, shape=(
                None, self.num_layers * 2 * self.lstm_size), name="lstm_init_value")
            ...
            self.final_outputs = tf.reshape(tf.nn.softmax(
                network_output), (batch_time_shape[0], batch_time_shape[1], self.out_size))
...
def LoadData(path='data/shakespeare.txt'):
        # Load the data
    data_ = ""
...
def InitModel(vocab_):
    in_size = out_size = len(vocab_)
    lstm_size = 256  # 128
    num_layers = 2
    ...
    return net, sess, saver


# 1) TRAIN THE NETWORK

def TrainModel(net, sess, saver, save_path="saved/model.ckpt"):
    batch_size = 64  # 128
    time_steps = 100  # 50
...
def LoadModel(saver, sess, ckpt_file):
    saver.restore(sess, ckpt_file)


def GenText(net, vocab, TEST_PREFIX):
    ...
    for i in range(LEN_TEST_TEXT):
        # Sample character from the network according to the generated output
        # probabilities
...
if __name__ == '__main__':
    ckpt_file = "../data/models/shakespeare/model.ckpt"
    ...
    data, vocab = LoadData(path='../data/models/shakespeare/train.txt')

    net, sess, saver = InitModel(vocab)

    if ckpt_file == "":
        ckpt_file = TrainModel(net, sess, saver)

    if ckpt_file != "":
        LoadModel(saver, sess, ckpt_file)

    print "SENTENCE:"
    print GenText(net, vocab, TEST_PREFIX)
